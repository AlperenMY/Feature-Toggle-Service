...
1. Open Grafana at http://localhost:3001 and login (admin/admin).
2. Add Prometheus datasource (first-time setup):
   - Go to **Connections** -> **Add new connection**.
   - Search for **Prometheus** and click **Add new data source**.
   - Set URL to `http://prometheus:9090`.
   - Click **Save & test**.
3. Go to **Dashboards** -> **New** -> **Import**.
4. Upload `grafana/feature-toggle-dashboard.json`.
...
- List endpoint caches _raw flag list_ per `(tenant, env)` as `features:{tenant}:{env}` with TTL
